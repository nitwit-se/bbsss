...
function BBSLoop() {
    ...
    function convertCP437_to_UTF( str )  {
        newstr = '';
        for( i = 0; i < str.length; i++ ) {
            newstr += cp437_mapping[str[i]];
        }
        return newstr;
    }


    function delayedTextWriter(term, text, chunksize) {
        term.write( text.slice(0,chunksize) );
        if ( text != "" ) 
            setTimeout( function() { delayedTextWriter(term, text.slice(chunksize), chunksize) }, 5*time_scale );
        else
            setTimeout( function() { term.scrollToBottom(); delayedPostambleWriter(term, 0) }, 200*time_scale );
    }

    function delayedPreambleWriter(term, text, index, chunksize) {
        if ( index < preamble.length ) {
            if ( index == 8 ) { 
                term.writeln( preamble[index] + " " + baud_rate );
            } else {
                term.writeln( preamble[index] )
            }
            mx = 10;
            delay=50*mx; 
            if ( index < 3 ) delay=time_scale*mx;
            if ( index == 4 ) delay=time_scale*mx;
            if ( index == 5 ) delay=time_scale*mx;
            if ( index == 6 ) delay=time_scale*mx;
            if ( index == 7 ) delay=20*time_scale*mx;
            if ( index == 8 ) delay=15*time_scale*mx;
            if ( index == 10 ) delay=12*time_scale*mx;
            setTimeout( function() { delayedPreambleWriter(term, text, index+1, chunksize) }, delay );
        } else {

            setTimeout( function() { term.clear(); delayedTextWriter( term, text, chunksize ) }, 175*time_scale );
        }
    }

    function delayedPostambleWriter(term, index) {
        if ( index < postamble.length ) {
            term.write( postamble[index] )
            delay=500; 
            if ( index == 0 ) delay=10*time_scale;
            if ( index == 1 ) delay=200*time_scale;
            if ( index == 2 || index == 3 || index == 4 ) delay=10*time_scale;
            if ( index == 5 ) delay=50*time_scale;
            if ( index == 6 ) delay=25*time_scale;
    ...
    //key = keys[ keys.length * 0.1 << 0];
    let str = convertCP437_to_UTF( window.Base64.decode(jsondata[key]));
    if ( str.length > 10000 ) {
        time_scale = 4.0;
        baud_rate = 28800;
    ...
    $('#filename').html( key );
    $('#baud_rate').html(baud_rate );
    $('#clock').html( new Date().toLocaleTimeString() );

    document.title = "BBS Nostalgia";
    //let str = convertCP437_to_UTF( window.Base64.decode(jsondata['j_woolf.ans']));
    // Telemate / Telix 
    setTimeout( function() { delayedPreambleWriter( term, str, 0, 24 ); }, 200*time_scale );
